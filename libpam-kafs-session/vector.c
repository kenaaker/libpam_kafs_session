 ...
 */
struct vector * vector_copy(const struct vector *old) {
 ...
 */
bool vector_add(struct vector *vector, const char *string) {
 ...
 * memory allocation fails.
 */
static struct vector * vector_reuse(struct vector *vector) {
    if (vector == NULL)
        return vector_new();
    else {
        vector_clear(vector);
 ...
 * separators.
 */
static size_t split_multi_count(const char *string, const char *seps) {
    const char *p;
    size_t count;
 ...
 * does.
 */
int vector_exec(const char *path, struct vector *vector) {
 ...
 */
int vector_exec_env(const char *path, struct vector *vector,
